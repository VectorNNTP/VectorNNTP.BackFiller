...
namespace VectorNNTP.Backfiller.Tests
{
    ...
    public class TransitServerValidatorIsolationTests(ITestOutputHelper output)
    {
        ...
        public async Task TransitServer_UseSslMissing_DefaultFalse_DirectAndFullPipeline()
        {
            ...
            (ConfigurationValidationResult? configResult, DependencyValidationResult? dependencyResult, Configuration.BackFillerRuntimeOptions? runtimeOptions) = await StartupValidationPipeline.ValidateConfigurationDependenciesAndBuildRuntimeOptionsAsync(config, TimeSpan.FromSeconds(1), CancellationToken.None);
