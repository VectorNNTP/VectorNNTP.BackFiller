...
internal sealed class TransitTimingCollector
{
    ...
    {
        Interlocked.Increment(ref _publishPayloadCopyCount);
        Interlocked.Add(ref _publishPayloadCopyTotalTicks, elapsedTicks);
    }
    ...
    {
        Interlocked.Increment(ref _dotStuffStageCount);
        Interlocked.Add(ref _dotStuffStageTotalTicks, elapsedTicks);
        Interlocked.Add(ref _dotStuffPayloadBytes, payloadBytes);
        Interlocked.Add(ref _dotStuffGetSpanCalls, getSpanCalls);
        Interlocked.Add(ref _dotStuffAdvanceCalls, advanceCalls);
        Interlocked.Add(ref _dotStuffStuffedDotEvents, stuffedDotEvents);
    ...
    internal void RecordFlushWait(long elapsedTicks)
    {
        ...
        Interlocked.Add(ref _flushTotalTicks, elapsedTicks);
    ...
    {
        Interlocked.Increment(ref _responseLineReadCount);
        Interlocked.Add(ref _responseLineReadTotalTicks, elapsedTicks);
    ...
    {
        Interlocked.Increment(ref _responseCorrelationCount);
        Interlocked.Add(ref _responseCorrelationTotalTicks, elapsedTicks);
        ...
        {
            Interlocked.Increment(ref _responseAvailableToCorrelatedCount);
            Interlocked.Add(ref _responseAvailableToCorrelatedTotalTicks, responseAvailableToCorrelatedTicks);
        ...
        if (definitive)
        {
            Interlocked.Exchange(ref _lastDefinitiveResponseCorrelatedTick, correlatedTick);
        }
    }
    ...
    internal void RecordCompletionObserved(long completionEnqueuedTick, long workerObservedTick)
    {
        ...
        {
            Interlocked.Increment(ref _completionEnqueueToObserveCount);
            Interlocked.Add(ref _completionEnqueueToObserveTotalTicks, enqueueToObserveTicks);
        ...
        if (lastResponseTick > 0)
        {
            ...
            {
                Interlocked.Increment(ref _responseToWorkerObservationCount);
                Interlocked.Add(ref _responseToWorkerObservationTotalTicks, responseToObservationTicks);
        ...
        Interlocked.Exchange(ref _lastWorkerObservationTick, workerObservedTick);
    }

    internal void RecordWorkerPollDelay(long elapsedTicks)
    {
        Interlocked.Increment(ref _workerPollDelayCount);
        Interlocked.Add(ref _workerPollDelayTotalTicks, elapsedTicks);
    }
    ...
    internal void RecordStagingStarted(long stageStartTick)
    {
        ...
        if (lastObservationTick > 0)
        {
            ...
            {
                Interlocked.Increment(ref _workerObservationToNextStagingCount);
                Interlocked.Add(ref _workerObservationToNextStagingTotalTicks, workerObservationToStagingTicks);
        ...
        if (lastResponseTick > 0)
        {
            ...
            {
                Interlocked.Increment(ref _responseToNextStagingCount);
                Interlocked.Add(ref _responseToNextStagingTotalTicks, responseToStagingTicks);
