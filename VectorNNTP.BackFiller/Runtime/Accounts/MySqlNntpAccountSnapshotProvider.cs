...
internal sealed partial class MySqlNntpAccountSnapshotProvider
{
    ...
    private async Task<int> LoadAndPublishSnapshotAsync(CancellationToken cancellationToken)
    {
        ...
        ImmutableArray<NntpAccountSnapshot> immutableAccounts = loadedAccounts.ToImmutableArray();
...
#pragma warning restore CA2007
        ...
        command.Parameters.Add(new MySqlParameter("@ServerId", MySqlDbType.UByte) { Value = _serverId });
...
#pragma warning restore CA2007
    ...
    {
        Task EnsureDatabaseAndTableAsync(string databaseName, string tableName, string createTableSql, CancellationToken cancellationToken);
    }
